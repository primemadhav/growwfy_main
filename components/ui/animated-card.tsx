...
export function AnimatedCard({ className, ...props }: CardProps) {
  ...
  React.useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      {
        rootMargin: "0px 0px -15% 0px", // triggers when the card is in the top 85% of the viewport (including the center and below it)
        threshold: 0.1,
      }
    );
  ...
  return (
    <div
      ...
      className={cn(
        "group/animated-card relative w-[410px] overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-900 dark:bg-black isolate",
        isActive && "is-active",
        className
      )}
      {...props}
...
export function CardVisual({ className, ...props }: CardProps) {
  return (
    <div
      className={cn(
        "h-[180px] w-[410px] overflow-hidden bg-slate-50/50 dark:bg-zinc-950/40 rounded-t-xl isolate",
        className
      )}
      {...props}
    />
...
export function Visual1({
  ...
  // Dynamically match user's light/dark colors request
  const effectiveMainColor = isDark ? mainColor : "#ff6900";
  const effectiveSecondaryColor = isDark ? secondaryColor : "#ff4500";
  const effectiveGridColor = isDark ? gridColor : "rgba(0, 0, 0, 0.05)";

  return (
    <div
      aria-hidden
      className="relative h-full w-full overflow-hidden rounded-t-xl isolate"
    >
      <Layer1 color={effectiveMainColor} secondaryColor={effectiveSecondaryColor} />
      <Layer2 color={effectiveMainColor} />
      <Layer3 color={effectiveMainColor} secondaryColor={effectiveSecondaryColor} />
      <EllipseGradient color={effectiveMainColor} />
      <GridLayer color={effectiveGridColor} />
    </div>
  );
}
...
const EllipseGradient = ({ color }: EllipseGradientProps) => {
  return (
    <div className="absolute inset-0 z-[5] flex h-full w-full items-center justify-center">
      ...
      >
        <rect width="410" height="180" fill="url(#paint)" />
        <defs>
          <radialGradient
            id="paint"
            cx="0"
            ...
            gradientTransform="translate(205 98) rotate(90) scale(98 205)"
          >
...
const Layer1 = ({ color, secondaryColor }: LayerProps) => {
  return (
    <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute top-0 left-6 z-[6] transform transition-transform duration-500 group-hover/animated-card:-translate-x-[304px] group-[.is-active]/animated-card:-translate-x-[304px]">
      <svg
        className="w-[712px]"
        viewBox="0 0 712 180"
        fill="none"
...
const Layer2 = ({ color }: LayerProps) => {
  return (
    <div className="absolute top-0 left-6 h-full w-[410px]">
      <svg
        className="h-full w-[410px]"
        viewBox="0 0 410 180"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g clipPath="url(#clip0_25_384)">
          <path
            d="M1 131.5L33.5 125.5L64 102.5L93.5 118.5L124.5 90L154 100.5L183.5 76L207.5 92L244.5 51L274.5 60.5L307.5 46L334.5 28.5L410.5 1"
            stroke={color}
            strokeWidth="1.5"
          />
          <path
            d="M33.5 125.5L1 131.5V197H410.5V1L335 28.5L306.5 46L274.5 60.5L244.5 51L207.5 92L183.5 76L154 100.5L124.5 90L93.5 118.5L64 102.5L33.5 125.5Z"
            fill={color}
            fillOpacity="0.15"
          />
        </g>
      ...
      <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[3] transform bg-gradient-to-r from-transparent from-0% to-slate-50/90 to-15% transition-transform duration-500 group-hover/animated-card:translate-x-full group-[.is-active]/animated-card:translate-x-full dark:to-zinc-950/90"></div>
    </div>
  );
};
...
const Layer3 = ({ color, secondaryColor }: LayerProps) => {
  return (
    ...
    >
      <div className="flex shrink-0 items-center rounded-full border border-zinc-200/80 bg-white px-2 py-0.5 shadow-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0 group-[.is-active]/animated-card:opacity-0 dark:border-zinc-800/80 dark:bg-zinc-900">
        <div className="h-1.5 w-1.5 rounded-full bg-[var(--color)] animate-pulse" />
        <span className="ml-1.5 text-[10px] font-semibold text-zinc-800 dark:text-zinc-200">
          Profit
        </span>
      </div>
      <div className="flex shrink-0 items-center rounded-full border border-zinc-200/80 bg-white px-2 py-0.5 shadow-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0 group-[.is-active]/animated-card:opacity-0 dark:border-zinc-800/80 dark:bg-zinc-900">
        <div className="h-1.5 w-1.5 rounded-full bg-[var(--secondary-color)] animate-pulse" />
        <span className="ml-1.5 text-[10px] font-semibold text-zinc-800 dark:text-zinc-200">
          Traffic
        </span>
      </div>
...
const Layer4 = () => {
  return (
    <div className="group relative h-full w-[410px]">
      <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[7] flex max-w-[410px] -translate-y-full items-start justify-start bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0 group-[.is-active]/animated-card:translate-y-0">
        <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] rounded-md border border-zinc-200/80 bg-white/95 p-2 shadow-md opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100 group-[.is-active]/animated-card:opacity-100 dark:border-zinc-800/80 dark:bg-zinc-900/95">
          <p className="mb-0.5 text-xs font-bold text-zinc-900 dark:text-zinc-100">
